...
class Fitness_Function:
    ...
    def check_every_traversability(self):
        ...
        return self.resultBitmap.is_path_existent(firstDoor, targets, visited_out=self._reachable_cache, exhaustive=True)
    
    def poop_fire_reachability_score(self):
        ...
        for x in range(self.resultBitmap.width):
            for y in range(self.resultBitmap.height):
                ent_id = self.resultBitmap.get_entity_id_with_pixel_value(
                            self.resultBitmap.bitmap.getpixel((x, y))
                        )
                if ent_id in (EntityType.POOP, EntityType.FIRE):
                    poop_fire.append((x, y))

        if not poop_fire:
            return 1.0

        reachable_count = sum(1 for p in poop_fire if p in self._reachable_cache)
        return reachable_count / len(poop_fire)
    ...
    def calc_fitness_function(self):
        value = 0
        # total_weight = Constants.FITNESS_WEIGHT_BALANCE + Constants.FITNESS_WEIGHT_CHANGES + Constants.FITNESS_WEIGHT_ENEMIES + Constants.FITNESS_WEIGHT_SYMMETRY + Constants.FITNESS_WEIGHT_VARIATION + Constants.FITNESS_WEIGHT_POOP_FIRE
        total_weight = Constants.FITNESS_WEIGHT_BALANCE + Constants.FITNESS_WEIGHT_ENEMIES + Constants.FITNESS_WEIGHT_SYMMETRY + Constants.FITNESS_WEIGHT_VARIATION + Constants.FITNESS_WEIGHT_POOP_FIRE
        if(not self.check_every_traversability() ):
            self.functionValue = [0, 0, 0, 0, 0, 0]
        # elif(not self.check_spawn()):
        #     self.functionValue = [0, 0, 0, 0, 0, 0, 0]
        else:
            ...
            poop_fire = self.poop_fire_reachability_score()
            value = (
                ...
                + Constants.FITNESS_WEIGHT_POOP_FIRE * poop_fire
            )
