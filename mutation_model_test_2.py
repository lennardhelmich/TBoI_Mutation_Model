...
from fitness_function_2 import Fitness_Function  # <--- Angepasst!
...
def get_test_bitmaps():
    ...
    inv_dir = Path("Bitmaps/Inverted_Mutations2") / bmp1.stem
...
def get_two_inverted_mutations():
    ...
    """
    base_dir = Path("Bitmaps/Inverted_Mutations2")
...
def evaluate_generated_bitmaps_normal_inverted(ckpt_gan, ckpt_vae, a_min = 0.2, a_max = 0.8, n=100):
    ...
    for i in range(n):
        ...
        # GAN generieren
        out_gan = f"interpolation_test_bmp_inv_2/temp_gan_{i}.bmp"
        test_gan_interpolation_alpha(ckpt_gan, bmp1, bmp2, alpha=alpha, out_path=out_gan)
        gan_bitmap = Image.open(out_gan)

        # VAE generieren
        out_vae = f"interpolation_test_bmp_inv_2/temp_vae_{i}.bmp"
        ...
        print(f"[{i+1}/{n}] GAN: {gan_fitness.functionValue[0]} | VAE: {vae_fitness.functionValue[0]}")
...
def evaluate_generated_bitmaps_inverted_inverted(ckpt_gan, ckpt_vae, n=100):
    ...
    for i in range(n):
        ...
        # GAN generieren
        out_gan = f"interpolation_test_inv_inv_2/temp_gan_{i}.bmp"
        test_gan_interpolation_alpha(ckpt_gan, bmp1, bmp2, alpha=alpha, out_path=out_gan)
        gan_bitmap = Image.open(out_gan)

        # VAE generieren
        out_vae = f"interpolation_test_inv_inv_2/temp_vae_{i}.bmp"
        test_vae_interpolation_alpha(ckpt_vae, bmp1, bmp2, alpha=alpha, out_path=out_vae)
        vae_bitmap = Image.open(out_vae)
...
def print_score_summary(gan_scores, vae_scores):
    ...
    Ignoriert alle Einträge mit Gesamtwert 0."""
    import numpy as np

    gan_arr = np.array(gan_scores)
    vae_arr = np.array(vae_scores)
    ...
    fitness_names = [
        ...
        "Symmetry",
        "Variation",
        "Poop/Fire Reachability"
    ]

    # Filter: Nur Werte mit Gesamtwert != 0
    ...
    if len(gan_arr) > 0 and len(vae_arr) > 0:
        diff = gan_arr[:, 0].mean() - vae_arr[:, 0].mean()
        print(f"Durchschnittlicher Unterschied (GAN - VAE, Gesamtwert): {diff:.4f}")
    else:
        print("Nicht genug gültige Werte für Vergleich.")

def save_scores_json(gan_scores, vae_scores, out_name):
    import os, json
    os.makedirs("evaluation_result_2", exist_ok=True)
    with open(f"evaluation_result_2/{out_name}_gan_scores.json", "w") as f:
        json.dump(gan_scores, f)
    with open(f"evaluation_result_2/{out_name}_vae_scores.json", "w") as f:
        json.dump(vae_scores, f)
    print(f"Saved: evaluation_result_2/{out_name}_gan_scores.json, evaluation_result_2/{out_name}_vae_scores.json")
...
# gan_scores, vae_scores = evaluate_generated_bitmaps_normal_inverted("optuna_gan_final_2/best_final.ckpt", "best_optuna_vae_model_2/best.pt", n=1000)
# gan_scores_2, vae_scores_2 = evaluate_generated_bitmaps_inverted_inverted("optuna_gan_final_2/best_final.ckpt", "best_optuna_vae_model_2/best.pt", n=1000)

# print_score_summary(gan_scores, vae_scores)
# print_score_summary(gan_scores_2, vae_scores_2)

def run_evaluation_series():
    gan_scores, vae_scores = evaluate_generated_bitmaps_normal_inverted("optuna_gan_final_2/best_final.ckpt", "best_optuna_vae_model_2/best.pt", a_min=0.0, a_max=1.0, n=100)
    save_scores_json(gan_scores, vae_scores, "alpha_00_10")
    gan_scores2, vae_scores2 = evaluate_generated_bitmaps_normal_inverted("optuna_gan_final_2/best_final.ckpt", "best_optuna_vae_model_2/best.pt", a_min=0.1, a_max=0.9, n=1000)
    save_scores_json(gan_scores2, vae_scores2, "alpha_01_09")
    gan_scores3, vae_scores3 = evaluate_generated_bitmaps_normal_inverted("optuna_gan_final_2/best_final.ckpt", "best_optuna_vae_model_2/best.pt", a_min=0.2, a_max=0.8, n=1000)
    save_scores_json(gan_scores3, vae_scores3, "alpha_02_08")
    gan_scores4, vae_scores4 = evaluate_generated_bitmaps_normal_inverted("optuna_gan_final_2/best_final.ckpt", "best_optuna_vae_model_2/best.pt", a_min=0.3, a_max=0.7, n=1000)
